...
const ShowTask = () => {
    ...
    const activeLists = mylists.filter(data => data.IsActive === true)

    const competeTask = (id) => {
        console.log(id)
        fetch(`https://taskbox-delta.vercel.app/lists/${id}`)
            .then(res => res.json())
        ...
        <div className='w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-5'>
            {
                activeLists.map(data => <>
                    <div className='bg-stone-300 w-full p-3 rounded-lg flex flex-col items-center'>
                        ...
                        <button className='bg-slate-400 hover:bg-sky-900  text-center text-white font-bold py-3 m-2 px-10 w-full' onClick={() => competeTask(data._id)}>Done</button>
                    </div>
                </>)
            }
        </div>
